...
export class Quaternion implements ReadOnlyQuaternion {
   ...
   */
  public static FromEulerAnglesRef(
    x: number,
    y: number,
    z: number,
    result: Quaternion
  ): void {
    return Quaternion.RotationYawPitchRollToRef(
      y * DEG2RAD,
      x * DEG2RAD,
      z * DEG2RAD,
      result
  ...
  public static FromToRotation(
    ...
    const w =
      Math.sqrt(v0.lengthSquared() * v1.lengthSquared()) + Vector3.Dot(v0, v1)
  ...
  public get eulerAngles() {
    ...
    if (test > 0.4995 * unit) {
      ...
      out.y = Math.atan2(
        2 * this.w * this.y + 2 * this.z * this.x,
